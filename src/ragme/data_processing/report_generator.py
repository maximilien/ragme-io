...
class ReportGenerator:
    ...
    def create_processed_file(self, file_path: str, results: dict[str, Any]):
        ...
        """
        processed_file_path = Path(file_path).with_suffix(
            Path(file_path).suffix + ".processed"
        )
    ...
    def _generate_human_readable_summary(self, results: dict[str, Any]) -> str:
        ...
        lines.append(
            f"Processing Date: {results.get('processing_start_time', 'Unknown')}"
        )
        lines.append("")
        ...
        if results.get("file_type") == "document":
            ...
            lines.append(
                f"Average Chunk Size: {results.get('average_chunk_size_kb', 0):.2f} KB"
            )
            ...
            if extracted_images:
                ...
                for i, img in enumerate(extracted_images):
                    ...
                    lines.append(
                        f"    EXIF Extracted: {'✅' if img.get('exif_extracted', False) else '❌'}"
                    )
                    lines.append(
                        f"    AI Classifications: {img.get('ai_classification_features', 0)}"
                    )
                    lines.append(
                        f"    OCR Success: {'✅' if img.get('ocr_success', False) else '❌'}"
                    )
                    if img.get("ocr_text_length", 0) > 0:
                        lines.append(
                            f"    OCR Text Length: {img.get('ocr_text_length', 0)} chars"
                        )
                    if img.get("errors"):
        ...
        # Image-specific results
            ...
            lines.append("-" * 20)
            lines.append(
                f"EXIF Extracted: {'✅' if results.get('exif_extracted', False) else '❌'}"
            )
            lines.append(
                f"AI Classifications: {results.get('ai_classification_features', 0)}"
            )
            lines.append(
                f"OCR Success: {'✅' if results.get('ocr_success', False) else '❌'}"
            )
            if results.get("ocr_text_length", 0) > 0:
                lines.append(
                    f"OCR Text Length: {results.get('ocr_text_length', 0)} chars"
                )

            if results.get("is_extracted"):
                lines.append(
                    f"Extracted from: {results.get('source_document', 'Unknown document')}"
                )
    ...
    def aggregate_results(self, results_list: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for result in results_list:
            ...
            if file_type == "document":
                ...
                stats["total_extracted_images"] += len(
                    result.get("extracted_images", [])
                )
        ...
        if stats["document_times"]:
            stats["avg_document_time"] = sum(stats["document_times"]) / len(
                stats["document_times"]
            )
        else:
            stats["avg_document_time"] = 0

        if stats["image_times"]:
            stats["avg_image_time"] = sum(stats["image_times"]) / len(
                stats["image_times"]
            )
        else:
        ...
        if stats["file_sizes"]:
            stats["avg_file_size_kb"] = sum(stats["file_sizes"]) / len(
                stats["file_sizes"]
            )
        else:
    ...
    def create_csv_report(
        ...
        filename: str = "processing_results.csv",
    ):
        """
            ...
            with open(csv_path, "w", newline="", encoding="utf-8") as csvfile:
                ...
                for result in results_list:
                    ...
                    if result.get("file_type") == "image":
                        ...
                        ai_classification_features = result.get(
                        ...
                        if extracted_images:
                            exif_extracted = any(
                                img.get("exif_extracted", False)
                                for img in extracted_images
                            )
                            ai_classification_features = sum(
                                img.get("ai_classification_features", 0)
                                for img in extracted_images
                            )
                            ocr_success = any(
                                img.get("ocr_success", False)
                                for img in extracted_images
                            )
                            ocr_text_length = sum(
                                img.get("ocr_text_length", 0)
                                for img in extracted_images
                            )
                        else:
                    ...
                    writer.writerow(
                        {
                            ...
                            "chunk_count": result.get(
                                "chunk_count",
                                0 if result.get("file_type") == "document" else "",
                            ),
                            "avg_chunk_size_kb": round(
                                result.get("average_chunk_size_kb", 0), 2
                            )
                            ...
                            "processing_time_seconds": round(
                                result.get("timing", {}).get("total", 0), 3
                            ),
                            "retry_count": result.get("retry_count", 0),
    ...
    def print_summary(self, results_list: list[dict[str, Any]], verbose: bool = False):
        ...
        if total_retries > 0:
            ...
            print(
                f"Files requiring retries: {sum(1 for r in stats['retry_counts'] if r > 0)}"
            )
        ...
        if verbose and results_list:
            ...
            for result in results_list:
                ...
                print(
                    f"{status_emoji} {file_type_emoji} {result.get('file_name', 'Unknown')}"
                )

...
class DocumentProcessor:
    ...
    def __init__(self, batch_size: int = 3, retry_limit: int = 3):
        ...
        self.text_vdb = create_vector_database(
                ...
                if config.get("pdf_image_extraction.enabled", True):
                    ...
                    for img_index, img in enumerate(image_list):
                        try:
                            ...
                            min_size_kb = config.get(
                                "pdf_image_extraction.min_image_size_kb", 1
                            )
                            max_size_mb = config.get(
                                "pdf_image_extraction.max_image_size_mb", 10
                            )
                            ...
                            if pix.n < 5:  # Skip if not a good image format
                                ...
                                if min_size_kb <= img_size_kb <= (max_size_mb * 1024):
                                    ...
                                    temp_img_path = tempfile.mktemp(
                                        suffix=f"_p{page_num}_i{img_index}.png"
                                    )
                            ...
                            print(
                                f"Warning: Failed to extract image {img_index} from page {page_num}: {e}"
                            )
    ...
    def process_docx_file(self, docx_path: str) -> tuple[str, dict]:
        ...
        try:
            ...
            raise Exception(f"Failed to process DOCX file: {str(e)}") from e
    ...
    def process_document(self, file_path: str) -> dict[str, Any]:
        ...
        try:
            ...
            if file_ext == ".pdf":
                ...
                text, page_count, pdf_metadata, extracted_image_paths = (
                    self.process_pdf_with_fallback(file_path)
                )
                ...
                for img_path in extracted_image_paths:
                    try:
                        img_result = self.process_image(
                            img_path, is_extracted=True, source_document=file_name
                        )
                        results["extracted_images"].append(img_result)
                    except Exception as e:
                        results["errors"].append(
                            f"Failed to process extracted image {img_path}: {str(e)}"
                        )
                    finally:
                ...
                results["timing"]["image_processing"] = (
                    time.time() - image_processing_start
                )
            ...
            results["average_chunk_size_kb"] = (
                sum(len(chunk) for chunk in chunks) / len(chunks) / 1024
                if chunks
                else 0
            )
            ...
            self._store_document_chunks(
                file_path, file_name, chunks, results["metadata"]
            )
            store_time = time.time() - store_start

            results["timing"]["vdb_storage"] = store_time
            results["chunks"] = [
                {"index": i, "size_bytes": len(chunk)} for i, chunk in enumerate(chunks)
            ]
    ...
    def process_image(
        self, file_path: str, is_extracted: bool = False, source_document: str = None
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            ocr_success = "ocr_content" in processed_image and processed_image[
                "ocr_content"
            ].get("ocr_processing", False)

            results["exif_extracted"] = exif_success
            results["ai_classification_features"] = len(
                processed_image.get("classification", {}).get("classifications", [])
            )
            results["ocr_success"] = ocr_success
            results["ocr_text_length"] = len(
                processed_image.get("ocr_content", {}).get("extracted_text", "")
            )
            ...
            # Store in VDB
            store_start = time.time()
            self._store_image_in_vdb(
                image_url, file_name, processed_image, combined_metadata
            )
            store_time = time.time() - store_start
    ...
    def process_file_with_retry(
        self, file_path: str, max_retries: int = None
    ) -> dict[str, Any]:
        """
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    print(
                        f"Attempt {attempt + 1} failed for {Path(file_path).name}: {last_error}. Retrying..."
                    )
                    time.sleep(1)  # Brief pause before retry
                else:
                    print(
                        f"All {max_retries + 1} attempts failed for {Path(file_path).name}: {last_error}"
                    )
        # All retries failed
        return {
            "file_name": Path(file_path).name,

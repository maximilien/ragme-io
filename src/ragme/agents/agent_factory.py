...
class AgentFactory:
    ...
    def __init__(self, agents_dir: str | None = None):
        ...
        logger.info(
            f"Initialized AgentFactory with agents directory: {self.agents_dir}"
        )

    def create_agent(
        self, agent_config: dict[str, Any], ragme_instance: Any | None = None, **kwargs
    ) -> AbstractAgent:
        """
        ...
        logger.info(
            f"Creating agent '{name}' with role '{role}' and type '{agent_type}'"
        )
    ...
    def _load_from_file(
        ...
        **kwargs,
        ...
        return self._create_agent_instance(
            agent_class=agent_class, ragme_instance=ragme_instance, env=env, **kwargs
        )
    ...
    def _load_from_github(
        ...
        **kwargs,
        ...
        if parsed.netloc != "github.com":
            raise ValueError(
                f"Only github.com repositories are supported, got: {github_uri}"
            )
        ...
        if not full_file_path.exists():
            raise FileNotFoundError(
                f"Agent file not found in repository: {full_file_path}"
            )
    ...
    def _clone_or_update_github_repo(self, owner: str, repo: str, branch: str) -> Path:
        ...
        if repo_dir.exists():
            ...
            try:
                ...
                logger.warning(
                    f"Could not update repository {owner}/{repo}, using existing version"
                )
        else:
            ...
            try:
                ...
                raise RuntimeError(
                    f"Could not clone repository {owner}/{repo}: {e}"
                ) from e
        ...
        **kwargs,
        ...
        if not hasattr(module, class_name_simple):
            raise AttributeError(
                f"Class '{class_name_simple}' not found in module '{module_name}'"
            )
    ...
    def _get_class_from_module(self, module: Any, class_name: str) -> type:
        ...
        raise AttributeError("No agent class found in module")

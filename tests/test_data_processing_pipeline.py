...
class TestDocumentProcessingPipeline(unittest.TestCase):
    ...
    def test_discover_files_empty_directory(self, mock_report_gen, mock_processor):
        ...
        mock_processor_instance.is_supported_file.side_effect = lambda path: path.endswith(('.pdf', '.jpg'))
    ...
    def test_optimize_processing_order(self, mock_report_gen, mock_processor):
        ...
        optimized_names = set(Path(f).name for f in optimized_order)
        original_names = set(filename for filename, _ in file_info)
    ...
    def test_process_single_file_success(self, mock_report_gen, mock_processor):
        ...
        mock_processor_instance.process_file_with_retry.assert_called_once_with(test_file, pipeline.retry_limit)
        mock_report_gen_instance.create_processed_file.assert_called_once_with(test_file, result)
    ...
    def test_process_files_parallel(self, mock_thread_pool, mock_report_gen, mock_processor):
        ...
        with patch('src.ragme.data_processing.pipeline.as_completed') as mock_as_completed:

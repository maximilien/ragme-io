...
class TestOpenAIAdapter:
    ...
    @patch('src.ragme.agents.adapters.openai_adapter.OpenAI')
    @pytest.mark.asyncio
    async def test_openai_adapter_run_with_system_prompt(self, mock_openai_class, mock_config):
        ...
        # Create adapter and test
        adapter = OpenAIAdapter("test", "test", "gpt-4", system_prompt="You are a test assistant")
        response = await adapter.run("test query")

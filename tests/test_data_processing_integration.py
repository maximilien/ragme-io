...
class TestDataProcessingIntegration(unittest.TestCase):
    ...
    def test_pipeline_empty_directory(self, mock_config):
        ...
        try:
            with DocumentProcessingPipeline(empty_dir, batch_size=1, verbose=False) as pipeline:
                stats = pipeline.run()
    ...
    def test_pipeline_discover_files(self, mock_config):
        ...
        with DocumentProcessingPipeline(self.temp_dir, batch_size=1, verbose=False) as pipeline:
            files_to_process, already_processed = pipeline.discover_files()
        
        # Should find 4 supported files (2 docs + 2 images)
        supported_files = [f for f in files_to_process if Path(f).name in ["document1.pdf", "document2.docx", "image1.jpg", "image2.png"]]
        ...
        unsupported_found = any("unsupported.txt" in f or "README.md" in f for f in files_to_process)
    ...
    def test_pipeline_process_single_image(self, mock_image_processor, mock_config):
        ...
        with DocumentProcessingPipeline(self.temp_dir, batch_size=1, verbose=True) as pipeline:
            stats = pipeline.run()
    ...
    def test_pipeline_optimize_processing_order(self, mock_config):
        ...
        with DocumentProcessingPipeline(self.temp_dir, batch_size=2, verbose=False) as pipeline:
...
class TestDataProcessingEndToEnd(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if fixtures_dir.exists():
            ...
            if image_dir.exists():
                for img_file in image_dir.glob("*"):
                    if img_file.is_file() and img_file.stat().st_size < 512 * 1024:  # Skip large files (>512KB)
                        shutil.copy2(img_file, self.temp_dir)
    ...
    def test_end_to_end_fixture_processing(self, mock_config):
        ...
        try:
            with DocumentProcessingPipeline(self.temp_dir, batch_size=1, retry_limit=1, verbose=True) as pipeline:
                stats = pipeline.run()
            ...
            print(f"Processing completed: {stats['successful_files']}/{stats['processed_files']} files successful")

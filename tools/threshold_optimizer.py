...
class ThresholdOptimizer:
    ...
    def update_threshold(self, threshold: float) -> None:
        ...
        for i, line in enumerate(lines):
            if "text_relevance_threshold:" in line:
                lines[i] = (
                    f"  text_relevance_threshold: {threshold}  # Minimum similarity score for text documents (0.0 to 1.0)\n"
                )
                break
    ...
    def restart_backend(self) -> None:
        ...
        subprocess.run(
            ["./start.sh", "restart-backend"], check=True, capture_output=True
        )
        time.sleep(3)  # Wait for services to start

    def test_query(
        self, query: str, expected_contains: list = None
    ) -> tuple[bool, str]:
        ...
        try:
            ...
            if expected_contains:
                ...
                return success, response_text[:150] + "..." if len(
                    response_text
                ) > 150 else response_text
            else:
                return True, response_text[:150] + "..." if len(
                    response_text
                ) > 150 else response_text
...
def main():
    ...
    print("💾 Threshold has been updated in config.yaml")
